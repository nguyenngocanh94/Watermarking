...
int main(int argc, char const *argv[])
{
	...
	double x = watermarkbits;
	double v = sqrt(x);
	int heightWM = v;
	Mat background(heightWM,heightWM,CV_8UC1,Scalar::all(255));
	vector<int> vectorwm;
	int flag = 0;

	for (int i = 0;flag < watermarkbits; i++)
	{
		for (int j = 0; j <b-1; j++)
		{
...
/************************** contruct the image ****************************/
	Mat watermarkimg(heightWM,heightWM,CV_8UC1,Scalar::all(0));
	for (int i = 0, k = 0; i < heightWM; ++i)
	{
		for (int j = 0; j < heightWM; ++j)
		{
			if (vectorwm[k] == 0)
			{	
